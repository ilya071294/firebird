		...
		if (*pages)
		{
			...
			if (*pages)
			{
				...
				const bool bitsOk = pp_bits == new_pp_bits ||
					(pp_bits ^ new_pp_bits) == ppg_dp_empty && !(pp_bits & ppg_dp_empty);
				...
				if (!bitsOk)
				{
					...
					if (!(new_pp_bits & (ppg_dp_empty | ppg_dp_secondary)))
					{
						...
						// Such cases are not supposed to happen, so they should be
						// reported as errors.
					...
					// visible if the last sweep run has skipped these pages due to these flags.
